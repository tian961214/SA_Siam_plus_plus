...
def train(data_dir, train_imdb, val_imdb, model_save_path="./modelS/", use_gpu=True):
    ...
    #checkpoint = torch.load('/home/deeplearning/.torch/models/vgg16-397923af.pth')
    ...
    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad,net.parameters()), config.lr, config.momentum, config.weight_decay)
    # adjusting learning in each epoch
    scheduler = StepLR(optimizer, 25, 0.01)
    ...
    for i in range(config.num_epoch):
        ...
        print ("Epoch %d   training loss: %f, validation loss: %f" % (i+1, torch.mean(torch.stack(train_loss)), torch.mean(torch.stack(val_loss))))


if __name__ == "__main__":

    data_dir = "/home/deeplearning/baba/genzong/data/ILSVRC2015_VID/ILSVRC2015_crops/Data/VID/train"
    train_imdb = "/home/deeplearning/tian/siam-torch/ILSVRC15-curation/imdb_video_train.json"
    val_imdb = "/home/deeplearning/tian/siam-torch/ILSVRC15-curation/imdb_video_val.json"
