...
def run_tracker(p):
    ...
    net_S=torch.load(os.path.join(p.net_S_path,p.netS))
    net = net.to(device)
    ...
    z_features_4_S,z_features_5_S = net_S.feat_extraction(Variable(z_crop_tensor).to(device))
    z_feat4_attention = net_S.conv4_att(z_features_4_S)
    z_feat5_attention = net_S.conv5_att(z_features_5_S)
    z_feat4 = z_feat4_attention * z_features_4_S
    z_feat5 = z_feat5_attention * z_features_5_S
    z_feat4 = net_S.fuse_conv4(z_feat4)
    z_feat5 = net_S.fuse_conv5(z_feat5)
    z_feat4 = z_feat4.repeat(p.num_scale, 1, 1, 1)
    z_feat5 = z_feat5.repeat(p.num_scale, 1, 1, 1)
    ...
    for i in range(0, len(img_list)):
        if i > 0:
            ...
            x_features_4_S,x_features_5_S = net_S.feat_extraction(Variable(x_crops_tensor).to(device))


            x_feat4 = net_S.fuse_conv4(x_features_4_S)
            x_feat5 = net_S.fuse_conv5(x_features_5_S)
            #print x_feat4.shape
            #x_feat4 = x_feat4.repeat(p.num_scale, 1, 1, 1)
            #x_feat5 = x_feat5.repeat(p.num_scale, 1, 1, 1)

            #print x_feat4.shape
            # evaluate the offline-trained network for exemplar x features

            target_position, new_scale = tracker_eval(net, net_S,round(s_x), z_features, x_features,z_feat4,z_feat5,x_feat4,x_feat5, target_position, window, p)

            #target_position, new_scale = tracker_eval(net, round(s_x), z_features, x_features,  target_position, window, p)
            #target_position, new_scale = tracker_eval_S(net_S, round(s_x), z_features_S, x_features_S, target_position, window,p)
...
if __name__ == "__main__":
    ...
    if demp_type == "single":   # single video demo
        ...
        for video in all_videos:
            ...
            if p.bbox_output:
                #VOT
                #video_path = join('test', 'VOT2016', 'Ours',
                #                  'baseline', p.video.lower())
                #if not isdir(video_path): makedirs(video_path)
                #result_path = join(video_path, '{:s}_001.txt'.format(p.video.lower())
                #with open(result_path, "w") as fin:
                #    for x in bbox_result:

                #        fin.write(','.join([vot_float2str("%.4f", i) for i in x]) + '\n')

                #OTB
                #np.savetxt(p.bbox_output_path + p.video.lower() + '_001.txt', bbox_result, fmt='%.3f',delimiter=",")
                Mat = {'res': list(bbox_result), 'type': 'rect', 'fps': fps, 'len': len(bbox_result), 'annoBegin': 1,
                       'startFrame': 1}
                M = np.array([Mat])
                Mat2 = {'__header__': 'b', '__version__': 1.0, '__globals__': [], 'results': M}
                scio.savemat(p.bbox_output_path + p.video.lower() + '_Ours.mat', Mat2)
            fps_all = fps_all + fps
