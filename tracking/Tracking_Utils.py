...
lis1=['BlurBody','BlurCar2','BlurCar1','BlurFace','BlurOwl','Box','Car4','CarScale','ClifBar','Couple','Girl','Surfer','Sylvester','Walking','Walking2','Woman','BlurCar4','BlurCar3','Board',
      'Crossing','Dog','FaceOcc1','Jogging','Rubik','Singer1','Subway','Toy','Twinnings','Vase','Jogging_2','Skating2','Skating2_2']
def load_sequence(seq_root_path, seq_name):
    """
    load sequences;
    sequences should be in OTB format, or you can custom this function by yourself
    """
    img_dir = os.path.join(seq_root_path, seq_name, 'img/')     #OTB
    gt_path = os.path.join(seq_root_path, seq_name, 'groundtruth_rect.txt')   #OTB

    #img_dir = '/home/deeplearning/tian/siam-torch/data/UAV123/'+seq_name+'/'     #UAV123
    #gt_path = '/home/deeplearning/tian/siam-torch/data/UAV123anno/'+seq_name+'.txt'     #UAV123

    #img_dir =  '/home/deeplearning/tian/siam-torch/data/VOT2016/'+seq_name+'/' # VOT
    #gt_path = os.path.join(seq_root_path, seq_name, 'groundtruth.txt')  # VOT
...
def tracker_eval(net,net_S, s_x, z_features, x_features, z_feat4,z_feat5,x_feat4,x_feat5,target_position, window, p):
#def tracker_eval(net, s_x, z_features, x_features, target_position,window, p):
    ...
    xcorr_out4 = net_S.xcorr(z_feat4, x_feat4)
    xcorr_out5 = net_S.xcorr(z_feat5, x_feat5)
    scores_S = xcorr_out4 + xcorr_out5
    scores_S=scores_S.to('cpu')

    scores=labda*scores+(1-labda)*scores_S
